...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_film'])) {
    ...
    if (isset($_FILES['poster']) && $_FILES['poster']['error'] === UPLOAD_ERR_OK) {
        ...
        $targetDir = "/var/www/html/wowo/";
        $targetFile = $targetDir . basename($poster['name']);

        // Move the uploaded file to the target directory
        if (move_uploaded_file($poster['tmp_name'], $targetFile)) {
            chmod($posterPath, 0644);
            chown($posterPath, 'www-data');// Replace 'www-data' with your web server's user
              

            $posterPath = $poster['name']; // Save only the file name in the database
